...
public class Main02 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Danh sách lọc ra sinh viên có điểm lớn hơn 7:");
        List <Student> filtered = students.stream()
                .filter(student -> student.getGrade()>7).toList();
        filtered.forEach(Student ::display);
        List <Student> sorted = filtered.stream()
                .sorted(Comparator.comparing(Student::getName)).toList();
        System.out.println("Danh sách sinh viên có điểm lớn hơn 7 và sắp xếp theo tên:");
